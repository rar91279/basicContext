...
const valid = function (item = {}) {
    ...
    if (item.items && item.items instanceof Array)item.type = SUBMENU
    ...
    if (item.fn == null && item.type !== SEPARATOR && item.type !== SUBMENU && item.disabled === false) {

        console.warn(`Missing fn for item '${ item.title }'`)
        return false
    }
    // Submenus needs some submenu items
    if (item.type === SUBMENU && item.items.length === 0 && item.disabled === false) {
        console.warn(`Missing subitems for '${ item.title }'`)
        return false
    }
...
const buildItem = function (item, num) {
    ...
    if (item.type === ITEM) {
        ...
        let submenu_html = buildSub(item.items, num);
        let expan_icon_span = `<span class='basicContext__icon--exp ${ item.exp_icon }'></span>`
        html = `
		       <tr class='basicContext__item'>
		       		<td class='basicContext__data' data-num='${ item.num }'>
                        ${ span } 
		       		    <span class="basicContext__title">${ item.title }</span> ${ submenu_html } 
                        ${ expan_icon_span }
                    </td>
			   </tr>
		       `
...
const buildSub = function (items, num){
    let html = ''

    html += `
	            <div class='basicContext__submenu'>
	                <table>
	                    <tbody>
	        `
